...
<template>
  <div class="panel">
    <div class="head">
      <div class="title">导出任务 ({{ convert.jobs.length }})</div>
      <div class="actions">
        <NButton size="tiny" secondary :disabled="!convert.batchId" @click="convert.refreshJobs()">刷新</NButton>
        <NButton size="tiny" secondary :disabled="!convert.polling" @click="convert.stopPolling()">停止轮询</NButton>
      </div>
    </div>

    <NEmpty v-if="!convert.jobs.length" size="small" description="暂无任务" class="empty" />
    ...
    <div v-else class="wrap">
      <table class="table">
        ...
        <tbody>
          <tr v-for="j in convert.jobs" :key="j.id">
      ...
      <div class="foot">
        <div class="sum">
          ...
          <NProgress
            type="line"
            :percentage="convert.overallProgress"
            :show-indicator="true"
            :height="8"
            color="var(--pf-primary)"
            :rail-color="'#333'"
