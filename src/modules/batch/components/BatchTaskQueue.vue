...
<template>
  <div class="panel">
    <div class="head">
      <div class="title">导出任务队列 ({{ batch.jobs.length }})</div>
      <div class="actions">
        <NButton size="tiny" secondary :disabled="!batch.batchId" @click="batch.refreshJobs()">刷新</NButton>
        <NButton size="tiny" secondary :disabled="!batch.polling" @click="batch.stopPolling()">停止轮询</NButton>
      </div>
    </div>

    <NEmpty v-if="!batch.jobs.length" size="small" description="暂无任务" class="empty" />
    ...
    <div v-else class="wrap">
      <table class="table">
        ...
        <tbody>
          <tr v-for="j in batch.jobs" :key="j.id">
